...
type PdfProcessorClient interface {
	// 流式上传PDF并返回解析文本
	ExtractText(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[PdfRequest, PdfResponse], error)
}
...
func (c *pdfProcessorClient) ExtractText(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[PdfRequest, PdfResponse], error) {
...
type PdfProcessor_ExtractTextClient = grpc.BidiStreamingClient[PdfRequest, PdfResponse]
...
type PdfProcessorServer interface {
	// 流式上传PDF并返回解析文本
	ExtractText(grpc.BidiStreamingServer[PdfRequest, PdfResponse]) error
...
func (UnimplementedPdfProcessorServer) ExtractText(grpc.BidiStreamingServer[PdfRequest, PdfResponse]) error {
	return status.Error(codes.Unimplemented, "method ExtractText not implemented")
}
...
func RegisterPdfProcessorServer(s grpc.ServiceRegistrar, srv PdfProcessorServer) {
	// If the following call panics, it indicates UnimplementedPdfProcessorServer was
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type PdfProcessor_ExtractTextServer = grpc.BidiStreamingServer[PdfRequest, PdfResponse]
